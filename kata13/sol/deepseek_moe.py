"""
This file is derived from https://github.com/deepseek-ai/DeepSeek-V3.2-Exp/
...
class Linear(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            empty(out_features, in_features, dtype=dtype or Linear.dtype)
        )
        if self.weight.element_size() == 1:
            scale_out_features = (out_features + block_size - 1) // block_size
            scale_in_features = (in_features + block_size - 1) // block_size
            self.weight.scale = self.scale = nn.Parameter(
                empty(scale_out_features, scale_in_features, dtype=torch.float32)
            )
        else:
...
class Gate(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.weight = nn.Parameter(empty(args.n_routed_experts, args.dim))
        self.bias = (
            nn.Parameter(empty(args.n_routed_experts, dtype=torch.float32))
            if self.dim == 7168
            else None
        )
